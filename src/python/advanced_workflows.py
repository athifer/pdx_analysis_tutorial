...
class PDXWorkflows:
    ...
    def load_data(self):
        ...
        try:
            # Try realistic data first (generated by generate_realistic_pdx_data.py)
            if os.path.exists(f'{self.data_dir}/tumor_volumes_realistic.csv'):
                self.tumor_data = pd.read_csv(f'{self.data_dir}/tumor_volumes_realistic.csv')
                self.expression_data = pd.read_csv(f'{self.data_dir}/expression_tpm_realistic.csv')
                self.variant_data = pd.read_csv(f'{self.data_dir}/variants_realistic.csv')
                print(f"✓ Loaded realistic PDX data: {len(self.tumor_data)} tumor measurements, "
                      f"{self.expression_data.shape[1]-1} genes, {len(self.variant_data)} variants")
            # Try effective data as backup
            elif os.path.exists(f'{self.data_dir}/tumor_volumes_effective.csv'):
                self.tumor_data = pd.read_csv(f'{self.data_dir}/tumor_volumes_effective.csv')
                self.expression_data = pd.read_csv(f'{self.data_dir}/expression_tpm_effective.csv')
                self.variant_data = pd.read_csv(f'{self.data_dir}/variants_effective.csv')
                print(f"✓ Loaded effective PDX data: {len(self.tumor_data)} tumor measurements, "
                      f"{self.expression_data.shape[1]-1} genes, {len(self.variant_data)} variants")
            # Fall back to old mock data naming
            else:
                ...
                print(f"✓ Loaded mock PDX data: {len(self.tumor_data)} tumor measurements, "
                      f"{self.expression_data.shape[1]-1} genes, {len(self.variant_data)} variants")
